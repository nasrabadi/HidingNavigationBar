...
open class HidingNavigationBarManager: NSObject, UIScrollViewDelegate, UIGestureRecognizerDelegate {
    ...
    //status bar
    fileprivate var hotspotEnabled:Bool {
        get {
    ...
    public init(viewController: UIViewController, scrollView: UIScrollView){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.statusBarFrameDidChanged(notification:)), name: NSNotification.Name.UIApplicationDidChangeStatusBarFrame, object: nil)
        
    }
    
    func statusBarFrameDidChanged(notification:Notification) {
